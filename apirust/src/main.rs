...
async fn download_file(user: &str, batch: &str, filename: &str) -> Option<NamedFile> {
    let file_path = format!("/opt/services/servidor/files/{}/{}/{}", user, batch, filename);
...
async fn download_batch(user: &str, batch: &str) -> io::Result<NamedFile> {
    let dir_path = format!("/opt/services/servidor/files/{}/{}", user, batch);
    let zip_file_path = format!("/opt/services/servidor/files/{}/{}.zip", user, batch);
...
fn decode_base64(message:String) -> Result<String, String> {
    ...
    let dir_user = format!("/opt/services/servidor/files/{}", object.userID);
